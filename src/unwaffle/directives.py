...
_JVM = [
    r"(?i)^checkstyle[:.]\s*(off|on)\b",         # Checkstyle audit fences
    r"^@formatter:(off|on)\b",                   # IntelliJ formatter fences
    r"^spotless:(off|on)\b",
    r"^noinspection\s",                          # IntelliJ
    r"^\$NON-NLS-\d+\$",                         # Eclipse i18n markers
]
...
_CSHARP = [
    r"^ReSharper (disable|restore)\b",
    r"^<auto-generated",                         # standard codegen header (also skips the file)
]
...
_SWIFT = [
    ...
    r"^MARK:",                                   # Xcode jump-bar sections (the #region of Swift)
]
